...
from gmail_mcp_server import _list_messages, _get_message  # noqa: E402
...
def fetch_recent_emails(count: int = 10) -> list[dict]:
    ...
    message_stubs = _list_messages(service, max_results=count)

    # Step 2: fetch full email object for each ID
    emails: list[dict] = []
    for stub in message_stubs:
        try:
            email_obj = _get_message(service, stub["id"])
