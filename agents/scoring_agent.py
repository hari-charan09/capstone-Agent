...
def check_lookalike(domain: str) -> str | None:
    ...
    for brand, legit_domain in LEGITIMATE_DOMAINS.items():
        ...
        # Or if the brand name is in the domain but the domain is not the legitimate one
        if brand in normalized and domain != legit_domain:
            return brand
    return None
...
def score_email(email: dict) -> dict:
    ...
    # If the email is risky (> 20) and "Too-Good-To-Be-True Offers" is present, categorize as scam
    is_scam = "Too-Good-To-Be-True Offers" in signals_language
    
    if total_score <= 20:
        category = "safe"
    elif total_score <= 50:
        category = "scam" if is_scam else "spam"
    else:
        category = "scam" if is_scam else "phishing"
